...
export function orderedBayerDither(
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      const biased: RGB = [r + t * 255 - 127, g + t * 255 - 127, b + t * 255 - 127].map(v => clamp(v)) as RGB;
...
export function floydSteinbergDither(
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      const i = idx(x, y);
      const old: RGB = [to01(out.data[i]), to01(out.data[i + 1]), to01(out.data[i + 2])] as RGB;
      const nq = nearestColor(old, palette);

      const err: RGB = [out.data[i] - nq[0], out.data[i + 1] - nq[1], out.data[i + 2] - nq[2]] as RGB;

      out.data[i] = nq[0]; out.data[i + 1] = nq[1]; out.data[i + 2] = nq[2]; out.data[i + 3] = 255;
